...
#include <sstream>
void g_commandhandle_on_data_callback(void* context,SRT_DATA_MSG_PTR data_ptr, uint32_t  media_type, uint64_t dts, uint64_t pts){
	YangCommandHandle* handle=(YangCommandHandle*)context;
	handle->on_data_callback(data_ptr,media_type,dts,pts);
}
YangCommandHandle::YangCommandHandle() {
	m_context = NULL;
	m_userData = NULL;
	m_dataCallback.context=this;
	m_dataCallback.on_data_callback=g_commandhandle_on_data_callback;
}
...
void YangCommandHandle::decodeMessage(uint8_t *p, int32_t plen) {
	...
	demux.decode(input_ptr, &m_dataCallback);
}
