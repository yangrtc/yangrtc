...
int32_t YangReceive::connectServer(int32_t puid) {
	if (getPlayer(puid)) {
        return getPlayer(puid)->connectSfuServer();
	}
	return 1;
}
...
int32_t YangReceive::playAudio(int32_t puid) {
    ...
    int32_t ret = getPlayer(puid)->connectSfuServer();
...
int32_t YangReceive::unPlayAudio(int32_t puid) {
	...
	if (index > -1) {
        YangPeerConnection2 *sh = m_players.at(index);
		if (sh) {
...
void YangReceive::disConnectAll() {
	for (size_t i = 0; i < m_players.size(); i++) {
        YangPeerConnection2 *sh = m_players.at(i);
        sh->close();
        yang_delete(sh);
	}
	m_players.clear();
...
void YangReceive::disConnect(int32_t puid) {
	for (size_t i = 0; i < m_players.size(); i++) {
        if (m_players.at(i)->streamConfig->uid == puid) {
            YangPeerConnection2 *sh = m_players.at(i);
            sh->close();
            yang_delete(sh);
...
//	}
	...
	for (int32_t index = 0; index < (int) m_players.size(); index++) {
        if (m_players.at(index)->streamConfig->uid == puid) {
            YangPeerConnection2* sh=m_players.at(index);
           // if (sh->context->audioStream || sh->context->videoStream)		return;
            //sh->context->streamInit = 0;
            sh->close();

            yang_delete(m_players.at(index));
...
YangPeerConnection2* YangReceive::getPlayer(int32_t puid) {
	for (size_t i = 0; i < m_players.size(); i++) {
        if (m_players.at(i)->streamConfig->uid == puid)
...
int32_t YangReceive::reconnect() {
	...
	for (size_t i = 0; i < m_players.size(); i++) {
        if (!m_players.at(i)->isConnected()) {
            retCode = m_players.at(i)->connectSfuServer();
			if (retCode)
				ret = retCode;
		}
