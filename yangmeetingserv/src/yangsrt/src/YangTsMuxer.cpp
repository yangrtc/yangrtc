...
void YangTsMuxer::create_pes(YangTsPes *frame, uint8_t *p, int plen,
		int frametype, long long timestamp, YangTsStream pstreamType) {
	YangTsBuffer packet;
	PESHeader pes_header;
	uint8_t streamType = STREAM_TYPE_VIDEO_H264;
	if (pstreamType == TS_H264) {
		streamType = 0xe0;   //STREAM_TYPE_VIDEO_H264;
		frame->pid = PES_VIDEO_ID;
		frame->pts = frame->dts = timestamp;
	}
	if (pstreamType == TS_H265) {
		streamType = 0xe0;   //STREAM_TYPE_VIDEO_H265;
		frame->pid = PES_VIDEO_ID;
		frame->pts = frame->dts = timestamp;
	}
	if (pstreamType == TS_AAC) {
		streamType = 0xc0;   //STREAM_TYPE_AUDIO_AAC;
		frame->pid = PES_AUDIO_ID;
		frame->pts = frame->dts = timestamp;
	}
	if (pstreamType == TS_OPUS) {
		streamType = 0xc0;   //STREAM_TYPE_AUDIO_OPUS;
		frame->pid = PES_AUDIO_ID;
		frame->pts = frame->dts = timestamp;
	}
...
void YangTsMuxer::encode(uint8_t *p, int plen, int frametype,
		long long timestamp, YangTsStream streamType,
...
void YangTsMuxer::encodeWithPmt(uint8_t *p, int plen, int frametype,
		long long timestamp, YangTsStream streamType,
		vector<YangTsBuffer> *sb) {
	...
	//	sb->push_back(YangTsBuffer());
	//    uint8_t pat_pmt_cc = get_cc(0);
	//   create_pat(&sb->at(0), pmt_pid, pat_pmt_cc);
	//  create_pmt(&sb->at(0), stream_pid_map, pmt_pid, pat_pmt_cc);
	//}
	create_pes(frame, p, plen, frametype, timestamp, streamType);
	// printf("a%d,",frame->len);
	create_ts(frame, sb);
