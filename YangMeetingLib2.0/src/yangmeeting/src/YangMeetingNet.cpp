...
YangMeetingNet::YangMeetingNet(YangMeetingContext *pini) {
	m_ini = pini;
	m_connState = Yang_Ok;
...
void YangMeetingNet::initNet() {
	...
	if (m_userData == NULL) {
		...
		m_userData->m_ini = m_ini;
		m_userData->m_userList = m_userList;
		m_userData->m_meeting = &m_ini->meeting;
		m_userData->m_user = &m_ini->user;
	}
	if (m_dataConn == NULL) {
		m_dataConn = new YangCommandHandle();
		m_dataConn->m_ini=m_ini;
		m_dataConn->m_userData = m_userData;
		m_dataConn->init(m_ini->sys.dataServerIP, m_ini->sys.dataPort);
	}
	initMediaNet();
}
...
void YangMeetingNet::setNetworkState(int32_t pst) {
	...
	if (m_connState != Yang_Ok) {
		closeConnect();
		if (m_ini->receive)
			m_ini->receive->receiveError(m_connState);
	}
}
...
void YangMeetingNet::initMediaNet() {
	if (!m_receive_media) {
		m_receive_media = new YangReceive(m_ini);
	...
	if (!m_publish_media) {
		m_publish_media = new YangPublish(m_ini);
...
void YangMeetingNet::initList() {
	if (m_out_al == NULL)
		m_out_al = new YangAudioEncoderBuffer(m_ini->audio.audioCacheNum);
	if (m_out_vl == NULL)
