   ...
   // m_dTransparent(1.0)
{
    setFixedSize(m_Width, m_Height);
      ...
      //  m_title->setStyleSheet("QPushButton{border-style:none;}");
        m_label = new QLabel(this);
        m_close = new QPushButton();
        //m_close->setIcon(QIcon(":/image/u394.svg"));
        m_close->setText("关闭");
      ...
      //  m_widget->setStyleSheet("background-color: #303030;");
        m_widget->setWindowOpacity(0.5);
        //m_widget_1->setStyleSheet("background-color: #4F4F5F;");
      //  m_widget_2->setStyleSheet("background-color: #4F4F5F;");
...
void YangDeskTip::run(){
m_threadState=1;
    sleep(2);

    while(m_nDesktopHeight <= m_nWindowHeight){
        usleep(1000);
        move(m_point.x(), m_nDesktopHeight);
         m_nDesktopHeight ++;
    }
