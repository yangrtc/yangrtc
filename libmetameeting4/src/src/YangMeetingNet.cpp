...
void YangMeetingNet::initNet() {
	...
	if (m_dataConn == NULL) {
		...
		m_dataConn->init(m_context->avinfo.sys.dataServerIP, m_context->avinfo.sys.dataPort);
	}
	initMediaNet();
}
void YangMeetingNet::stopAll() {
	if(m_dataConn) m_dataConn->stop();
	//if (m_receive_media)		m_receive_media->stop();
	if (m_publish_media)
		m_publish_media->stop();
}
...
//		if(m_context->avinfo.sys.transType!=Yang_Webrtc){
...
void YangMeetingNet::initList() {
	if (m_out_al == NULL)
		m_out_al = new YangAudioEncoderBuffer(m_context->avinfo.audio.audioCacheNum*5);
	if (m_out_vl == NULL)
		m_out_vl = new YangVideoDecoderBuffer(m_context->avinfo.video.evideoCacheNum*5);
}
