...
int32_t YangPublish::disConnectMediaServer() {
	if (m_pushs.size() > 0) {
		...
		yang_destroy_stream(m_pushs.back());
	...
	while (m_isConvert == 1) {
		...
		if (m_isPubAudio && m_in_audioBuffer && m_in_audioBuffer->size() > 0) {
			...
			for (i = 0; i < m_pushs.size(); i++) {
				ret = m_pushs.at(i)->publishAudioData(m_pushs.at(i)->context,&data);
				if (ret&&!m_pushs.at(i)->isconnected(m_pushs.at(i)->context)) {
					m_pushs.back()->context->streamInit = 0;
					m_pushs.back()->disConnectServer(m_pushs.at(i)->context);
					yang_post_message(YangM_Sys_PushMediaServerError,m_pushs.back()->context->uid,NULL);
				}
			}
		}
