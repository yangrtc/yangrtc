...
void YangMeetingEncoder::initAudioEncoder() {
	if (m_out_auidoBuffer == NULL)
		m_out_auidoBuffer = new YangAudioEncoderBuffer(m_context->avinfo.audio.audioCacheNum*5);
	if (m_ae == NULL) {
...
void YangMeetingEncoder::initVideoEncoder() {
	if (m_out_videoBuffer == NULL)
		m_out_videoBuffer = new YangVideoEncoderBuffer(m_context->avinfo.video.evideoCacheNum*5);

	if(m_context&&m_context->avinfo.enc.createMeta){
	if (m_vmd == NULL)
		m_vmd = (YangVideoMeta*) calloc(1, sizeof(YangVideoMeta));
		YangEncoderFactory fac;
		YangVideoEncoderMeta *yvh = fac.createVideoEncoderMeta(&m_context->avinfo.video);
		yvh->yang_initVmd(m_vmd, &m_context->avinfo.video,&m_context->avinfo.enc);
		yang_delete(yvh);
	}
	if (m_ve == NULL) {
		//	YangEncoderFactory yf;
		m_ve = new YangVideoEncoderHandle(m_context,&m_context->avinfo.video);
		m_ve->setOutVideoBuffer(m_out_videoBuffer);
		m_ve->init();
		m_ve->setVideoMetaData(m_vmd);
	}
}
void YangMeetingEncoder::sendMsgToEncoder(YangRequestType req){
