...
YangMeetingMessageHandle::YangMeetingMessageHandle(YangMeetingContext* pcontext,YangReceiveMessageI* prec) {

	m_receive = prec;
	m_context=pcontext;
	m_videoMeeting = new YangMeetingHandleImpl(m_context);
...
void YangMeetingMessageHandle::handleMessage(YangSysMessage *mss) {
	...
	case YangM_Sys_Login:
		if(mss->uid==m_context->user.userId||mss->uid==0){
