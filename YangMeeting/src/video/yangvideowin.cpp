...
YangVideoWin::YangVideoWin(QWidget *parent):QWidget(parent)
{
...
void YangVideoWin::init1(){
    if(vwin1==nullptr) {
        vwin1=new YangVideoShowWin(this);
        vwin1->m_selectUser=m_selectUser;
        m_hb->addWidget(vwin1);
        vwin1->resize(YangVideoContext::mainWin->width(),YangVideoContext::mainWin->height());
        // vwin1->resize(this->width(),this->height());
    }

    initSelect();
    int wid=YangVideoContext::mainWin->width();
    int hei=YangVideoContext::mainWin->height();
...
void YangVideoWin::init2(){
    ...
    if(vwin2==nullptr){
        vwin2=new YangVideoShowWin(this);
        m_hb->addWidget(vwin2);
    }

    initSelect();
    int wid=YangVideoContext::mainWin->width()/2;
    int hei=YangVideoContext::mainWin->height();
    vwin1->resize(wid,hei);
...
void YangVideoWin::init3(){
    ...
    if(m_vb1==nullptr){
        ...
        if(vwin3==nullptr){
            vwin3=new YangVideoShowWin(this);
...
void YangVideoWin::init4(){
    ...
    if(m_vb2==nullptr){
        ...
        if(vwin4==nullptr){
            vwin4=new YangVideoShowWin(this);
