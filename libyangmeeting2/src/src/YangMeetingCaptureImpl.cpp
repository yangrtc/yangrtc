	...
	if (m_audioCapture == NULL) {
		...
		if(ret){
			if(ret==ERROR_SYS_NoAudioDevice||ret==ERROR_SYS_NoAudioCaptureDevice) {
				...
				m_context->user.micFlag=0;
				return ret;
			}
			m_context->user.micFlag=1;
		}
...
YangMeetingCaptureImpl::YangMeetingCaptureImpl(YangMeetingContext *pcontext) {
	m_context=pcontext;
...
void YangMeetingCaptureImpl::initVideo(){
	...
	if (m_videoCapture == NULL) {
		...
		m_videoCapture = cf.createVideoCapture(&m_context->video);//new YangVideoCapture(m_context);
       ...
       //  if(1) return;
		...
		m_out_videoBuffer->init(m_context->video.width,m_context->video.height,12);
		m_pre_videoBuffer->init(m_context->video.width,m_context->video.height,12);
