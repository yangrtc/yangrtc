...
void YangVideoThread::initPara(YangMeetingContext *pini){
    m_ini=pini;
    if(pini->avinfo.video.videoCaptureFormat==YangI420){
        m_captureFormat=YangI420;
        m_is16bit=0;
    }
    if(pini->avinfo.video.videoDecHwType==0) {
...
//            m_previewSyn->resetVideoClock();
            ...
            m_showWins[ind]->videoBuffer=m_previewBuffer;
    ...
    for(int i=0;i<m_showWins.size();i++){
        ...
        if(ys->uid==-1){
            ...
            if(ys->uid==m_ini->user.userId){
                  ...
                  ys->videoBuffer=m_previewBuffer;//m_previewSyn;
        ...
        if(ys->uid==m_ini->user.userId){
            if(ys->videoBuffer){
                uint8_t* p=ys->videoBuffer->getVideoRef(&m_frame);
