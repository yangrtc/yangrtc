...
void YangSelectUserVideo::setIndex(int pindex){
    ...
    vector<YangUser*> *yus=m_context->videoMeeting->getUserList();
    int32_t uid=yus->at(pindex)->userId;

    if(uid==*m_uid) {yus=NULL;this->hide();return;}
    if(m_videoHandle) m_videoHandle->removeVideo(uid);
    if(*m_uid>-1)         {
        qDebug()<<"select remove uid=="<<*m_uid;
        m_context->videoThread->delWd(*m_uid);
    }

    *m_uid=uid;
    if(uid==m_context->context->user.userId){
...
void YangSelectUserVideo::init(){
    ...
    if(yus==NULL) return;
        ...
        for(int j=0;j<(int)yus->size();j++){
           ...
           if(yu->userId==m_context->context->user.userId||yu->camFlag==2) m_list.push_back(QString(yu->userName));
           yu=NULL;
        }

        int32_t cou=m_list.size();
