...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    m_ini->streams.m_playBuffers=new vector<YangSynBuffer*>();
    yang_setLogLevle(m_ini->avinfo.sys.logLevel);
    yang_setLogFile(m_ini->avinfo.sys.hasLogFile);
    initConf();
    ...
    m_videoContext.context=m_ini;
    m_videoContext.mainWin=this;
    m_videoContext.userinfo=&m_userinfo;
    m_videoContext.meeting=&m_ini->meeting;
...
void MainWindow::initConf(){
    m_ini->avinfo.audio.usingMono=1;
    m_ini->avinfo.audio.sample=16000;
    m_ini->avinfo.audio.channel=1;
    m_ini->avinfo.audio.hasAec=1;
    m_ini->avinfo.audio.audioCacheNum=10;
    m_ini->avinfo.audio.audioCacheSize=10;
    m_ini->avinfo.audio.audioPlayCacheNum=5;

    m_ini->avinfo.video.videoCacheNum=10;
    m_ini->avinfo.video.evideoCacheNum=10;
    m_ini->avinfo.video.videoPlayCacheNum=10;
    m_ini->avinfo.video.videoEncoderFormat=YangI420;

    m_ini->avinfo.sys.isMultCamera=0;
...
void MainWindow::initIntoMeeting(int pret){
 qDebug()<<"******initIntoMeeting********************************************"<<pret;
    ...
    m_videos->m_videos[0]->vwin1->vuid=m_ini->user.userId;
     yang_add_winshowvideo(m_videos->m_videos[0]->vwin1->getVWinId(),m_ini->user.userId);
...
void MainWindow::on_MainWindow_customContextMenuRequested(const QPoint &pos)
{
  //  qDebug()<<"--------------------------start context"<<m_videoMeeting->m_state;
    ...
    int32_t strlen=m_ini->avinfo.sys.isMultCamera?11:10;
    strlen+=m_ini->avinfo.sys.isMultCamera?2:3;
