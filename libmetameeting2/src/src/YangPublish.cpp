...
int32_t YangPublish::connectMediaServer() {
	...
	if (m_pushs.size() == 0) {
			char s[128] = { 0 };
            m_pushs.push_back(sf.createStreamHandle(m_context->avinfo.sys.transType,	m_context->user.userId,m_context));
			if (m_context->avinfo.sys.transType==Yang_Rtmp){
					sprintf(s, "user%d", m_context->user.userId);
					strcpy(streamConf.serverIp,m_context->avinfo.sys.rtmpServerIP);
					streamConf.serverPort=m_context->avinfo.sys.rtmpPort;
			}else if(m_context->avinfo.sys.transType==Yang_Srt){
				sprintf(s, "%d.%d", m_context->meeting.meetingId, m_context->user.userId);
				strcpy(streamConf.serverIp,m_context->avinfo.sys.srtServerIP);
				streamConf.serverPort=m_context->avinfo.sys.srtPort;
			}else if(m_context->avinfo.sys.transType==Yang_Webrtc){
				sprintf(s, "user%d", m_context->user.userId);
				strcpy(streamConf.serverIp,m_context->avinfo.sys.rtcServerIP);
				streamConf.serverPort=m_context->avinfo.sys.rtcPort;
...
void YangPublish::startLoop() {
	...
	metaFrame.initVideo(m_context->avinfo.sys.transType);
