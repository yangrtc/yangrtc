...
#include "src/video/YangVideoWinHandleH.h"
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    this->showMaximized();
    m_isInit=0;
    m_context=new YangMeetingContext();
    m_context->init((char*)"yang_config.ini");
    m_context->streams.m_playBuffers=new vector<YangSynBuffer*>();
   ...
   // m_context->avinfo.video.videoCaptureFormat==YangYuy2;
    ...
    m_videoContext.context=m_context;
    m_videoContext.mainWin=this;
    m_videoContext.userinfo=&m_userinfo;
    m_videoContext.meeting=&m_context->meeting;


    m_ld=new LoginDialog(&m_videoContext,this);
...
//
    ...
    m_meetinghandle=new YangMeetingInfoHandle(&m_videoContext);
    m_table=NULL;

    m_videos=new YangVideos(&m_videoContext);
...
void MainWindow::initConf(){
    ...
    m_context->avinfo.audio.enableAec=yangtrue;
    m_context->avinfo.audio.audioCacheNum=10;
    m_context->avinfo.audio.audioCacheSize=10;
    m_context->avinfo.audio.audioPlayCacheNum=5;
    ...
    m_context->avinfo.sys.enableMultCamera=yangfalse;
    m_context->avinfo.audio.audioEncoderType=Yang_AED_OPUS;
    m_context->avinfo.audio.audioDecoderType=Yang_AED_OPUS;
    /**
...
void MainWindow::initVideoThread(){
    ...
    m_videoContext.videoThread=m_videoThread;
    m_context->receive=this;
    //m_ld->m_ini=m_ini;
    m_http=new YangHttpUtil(m_context);
    m_ld->m_http=m_http;
...
void MainWindow::intoMeeting(){
    yang_post_message(YangM_Sys_ConnectServer,m_context->user.userId,NULL);
   // initIntoMeeting();
}
void MainWindow::handleConnectMeeting(int pret){
    ...
    if(pret>0){
       ...
       // else
        ...
        if(!m_exitState) yang_post_message(YangM_Sys_ConnectServer,m_context->user.userId,NULL);
        return;
    }
}
void MainWindow::initIntoMeeting(int pret){
 qDebug()<<"******initIntoMeeting********************************************"<<pret;
    ...
    m_videos->m_videos[0]->vwin1->vuid=m_context->user.userId;
     yang_add_winshowvideo(m_videos->m_videos[0]->vwin1->getVWinId(),m_context->user.userId);
    if(m_context->user.roomAdmin==1||m_context->user.roomAdmin==3){
        yang_post_message(YangM_Sys_Pubaudio,m_context->user.userId,NULL);
        yang_post_message(YangM_Sys_Pubvideo,m_context->user.userId,NULL);
       // m_ini->user.micFlag=2;
       // m_ini->user.camFlag=2;
    }
    for(size_t i=0;i<m_videoMeeting->getUserList()->size();i++){
        if(m_videoMeeting->getUserList()->at(i)->userId!=m_context->user.userId&&m_videoMeeting->getUserList()->at(i)->micFlag==2){
...
void MainWindow::handleConnectMedia(YangSysMessage* psm,int pret){
    ...
    if(pret){
        ...
        if(pmd==YangM_Sys_Login&&psm->uid!=m_context->user.userId){
            YangUser *yu=getUser(psm->uid);
            if(yu){
             estr="用户"+QString(yu->userName)+"已进入教室:"+QString(m_context->meeting.meetingName);
              qDebug()<<estr;
             showMessage(estr);
            }
        }
        if(pmd==YangM_Sys_Logout&&psm->uid!=m_context->user.userId){
...
void MainWindow::intoMeetingList(){
    if(m_table==nullptr) {
        m_table=new MeetingTable(&m_videoContext,this);
        ...
        m_table->show();
        m_table->initTable();
        if(m_videos) m_videos->m_table=m_table;
    }
...
void MainWindow::initPreview(){
    if(m_isInit==0){
        ...
        m_videoThread->showWin(m_videos->m_videos[showVideo-1]->vwin1->getVWinId(),m_context->user.userId);
...
void MainWindow::on_MainWindow_customContextMenuRequested(const QPoint &pos)
{
  //  qDebug()<<"--------------------------start context"<<m_videoMeeting->m_state;
    ...
    int32_t strlen=m_context->avinfo.sys.enableMultCamera?11:10;
    strlen+=m_context->avinfo.sys.enableMultCamera?2:3;
